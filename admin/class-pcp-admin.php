...
class PCP_Admin {
    ...
    public static function init() {
        ...
        // Manual points adjustment column on Users list
    ...
    public static function page_dashboard() {
        ...
        <div class="wrap pcp-admin-wrap">
            <h1>🏆 PlayChamp Points — Dashboard</h1>
    ...
    public static function page_settings() {
        ...
        $d = PCP_Settings::defaults();
        $g = function( $k ) { return PCP_Settings::get( $k ); };
        ?>
        <div class="wrap pcp-admin-wrap">
            <h1>⚙️ PlayChamp Points — Settings</h1>
            <form method="post">
                ...
                <div class="pcp-settings-grid">
                        ...
                        <table class="form-table">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description">Referrer gets these points when friend just registers (no purchase needed)</p>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description">Referrer gets these points when referred friend makes their first purchase</p>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description">Bonus for the new user who signed up via referral link</p>
                                </td>
                            </tr>
                        </table>
                    </div>
                        ...
                        <table class="form-table">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description">0 = never expire. Default: 180 days (6 months)</p>
                                </td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="pcp-settings-section">
                        ...
                        <table class="form-table">
                            <tr>
                                <th>🥈 Pro Champ — Min Points</th>
                                <td>
                                    <input type="number" name="tier_pro_min" value="<?php echo esc_attr($g('tier_pro_min')); ?>" min="1" class="small-text"> lifetime points
                                </td>
                            </tr>
                            <tr>
                                <th>🥈 Pro Champ — Earn Multiplier</th>
                                <td>
                                    <input type="number" name="tier_pro_earn_multiplier" value="<?php echo esc_attr($g('tier_pro_earn_multiplier')); ?>" min="1" step="0.1" class="small-text">x
                                </td>
                            </tr>
                            <tr>
                                <th>🥇 Legend — Min Points</th>
                                <td>
                                    <input type="number" name="tier_legend_min" value="<?php echo esc_attr($g('tier_legend_min')); ?>" min="1" class="small-text"> lifetime points
                                </td>
                            </tr>
                            <tr>
                                <th>🥇 Legend — Earn Multiplier</th>
                                <td>
                                    <input type="number" name="tier_legend_earn_multiplier" value="<?php echo esc_attr($g('tier_legend_earn_multiplier')); ?>" min="1" step="0.1" class="small-text">x
                                </td>
                            </tr>
                            <tr>
                                <th>🥇 Legend — Free Shipping</th>
                                <td>
                                    <label>
                                        <input type="checkbox" name="tier_legend_free_shipping" value="1" <?php checked(1, $g('tier_legend_free_shipping')); ?>>
                                        Enable free shipping for Legend tier
                                    </label>
                                </td>
                            </tr>
                        </table>
    ...
    public static function page_log() {
        ...
        if ( $user_filter ) {
            ...
            $args[] = '%' . $wpdb->esc_like( $user_filter ) . '%';
        }

        $query = "SELECT l.*, u.user_login FROM {$table} l LEFT JOIN {$wpdb->users} u ON l.user_id=u.ID WHERE {$where} ORDER BY l.created_at DESC LIMIT %d OFFSET %d";
        $args[] = $per_page;
        $args[] = $offset;

        $rows  = ! empty( array_filter($args, 'is_string') ) || true
            ? $wpdb->get_results( $wpdb->prepare( $query, ...$args ) )
            : $wpdb->get_results( $query );

        $total = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$table} l LEFT JOIN {$wpdb->users} u ON l.user_id=u.ID WHERE {$where}", ...array_slice($args, 0, count($args)-2) ) );
    ...
    public static function page_customers() {
        ...
        $where  = $search ? $wpdb->prepare("WHERE u.user_login LIKE %s OR u.user_email LIKE %s", '%' . $wpdb->esc_like($search) . '%', '%' . $wpdb->esc_like($search) . '%') : '';
        $now    = current_time('mysql');

        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT u.ID, u.user_login, u.user_email,
                COALESCE(SUM(CASE WHEN l.points > 0 AND (l.expires_at IS NULL OR l.expires_at > %s) THEN l.points ELSE 0 END)
                        + SUM(CASE WHEN l.points < 0 THEN l.points ELSE 0 END), 0) AS balance,
        ...
        <div class="wrap pcp-admin-wrap">
            ...
            <div class="tablenav bottom">
                <div class="tablenav-pages">
                    <?php echo paginate_links(array('base' => add_query_arg('paged','%#%'), 'current'=>$page, 'total'=>$pages)); ?>
                </div>
            </div>
            <?php endif; ?>
    ...
    public static function render_users_column( $output, $column_name, $user_id ) {
        ...
        return "<strong>{$balance}</strong> pts <br><small>{$tier['label']}</small>";
    }
    ...
    public static function ajax_adjust_points() {
        ...
        if ( ! $user_id || $amount == 0 ) wp_send_json_error( array('message' => 'Invalid data') );
